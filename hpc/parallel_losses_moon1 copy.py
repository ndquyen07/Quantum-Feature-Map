...
class TrainingConfig:
    ...
    n_samples: int = 150
    test_size: int = 50
    results_dir: str = "/data/ndquyen/qfm/results_losses_moon"
    init_theta_dir: str = None  # Directory containing pre-computed init_theta files
    init_theta_depth: int = 1   # Depth of the init_theta to load
...
def main_parallel(config: Optional[TrainingConfig] = None, **kwargs) -> tuple:
    ...
    # Generate data
    X, y = make_moons(n_samples=config.n_samples, noise=0.15, random_state=42)
    ...
    # Load or generate init_theta
    if config.init_theta_dir is not None:
        # Load pre-computed init_theta from specified directory
        init_theta_filename = (
            f"{config.init_theta_dir}/shared_init_theta_"
            f"depth{config.init_theta_depth}_ansatz{config.type_ansatz}_run{config.run_id}.npy"
        )
        if os.path.exists(init_theta_filename):
            shared_init_theta = np.load(init_theta_filename)
            print(f"Loaded init_theta from: {init_theta_filename}")
            print(f"Init theta shape: {shared_init_theta.shape}, Expected: {num_params}")
            
            # Verify the loaded theta has correct size
            if len(shared_init_theta) != num_params:
                print(f"WARNING: Loaded init_theta size ({len(shared_init_theta)}) != expected ({num_params})")
                print(f"Generating new random init_theta...")
                shared_init_theta = np.random.uniform(-np.pi, np.pi, num_params)
        else:
            print(f"WARNING: Init theta file not found: {init_theta_filename}")
            print(f"Generating new random init_theta...")
            shared_init_theta = np.random.uniform(-np.pi, np.pi, num_params)
    else:
        # Generate new random init_theta
        shared_init_theta = np.random.uniform(-np.pi, np.pi, num_params)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--init_theta_dir", type=str, default=None,
                        help="Directory containing pre-computed init_theta files (e.g., /data/ndquyen/qfm/results_losses_moon)")
    parser.add_argument("--init_theta_depth", type=int, default=1,
                        help="Depth of the init_theta to load from init_theta_dir")
    ...
    config = TrainingConfig(
        ...
        init_theta_dir=args.init_theta_dir,
        init_theta_depth=args.init_theta_depth
    )
