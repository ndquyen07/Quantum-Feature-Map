...
LOSS_FUNCTIONS = ["pre_determine_inner", "log_likelihood_micro", "log_likelihood_macro", "trace_distance", 
...
class TrainingConfig:
    ...
    n_samples: int = 150
    test_size: int = 50
    results_dir: str = "results_losses_gauss_2class"
...
def main_parallel(config: Optional[TrainingConfig] = None, **kwargs) -> tuple:
    ...
    # Generate data
    X, y = make_gaussian_quantiles(mean=None, cov=0.5, n_samples=config.n_samples, n_features=2, n_classes=2, random_state=42)
